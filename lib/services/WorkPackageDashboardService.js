...
export function computeDashboardStats(workPackage) {
  ...
  items.forEach((item) => {
    ...
    if (status === "APPROVED") {
      ...
      totals.inProgress++;
    } else if (status === "IN_REVIEW" || status === "CHANGES_NEEDED") {
      totals.needsReview++;
    } else {
      totals.notStarted++;
    }
 ...
 */
export function computeDashboardCTA(stats) {
  if (stats.needsReview > 0) {
    return "Review Deliverables";
  }

  if (stats.completed === stats.total && stats.total > 0) {
    return "View Full Project";
  }

  return "View Current Phase";
}
